...
models:
  phi-3-vision-vllm:                           # Specifier carries the engine
    ...
    sizing:                                    # What the variant costs to run
      weightGb: 8                              # VRAM consumed by this variant's weights
      weightBits: 16                           # Nominal bits per stored parameter, fp16 here
      minKvCacheGb: 2                          # Minimum KV-cache allocation
...
- **`sizing`**: Everything the variant costs to run, grouped together. `weightGb` is the VRAM its weights consume, roughly 14 for a 7B model in FP16, and a quantized variant of the same model is smaller, which is why it belongs in its own entry. `minKvCacheGb` is the minimum KV-cache allocation, which bounds how many concurrent requests it can serve. See [model-fields](./docs/model-fields.md) for the rest.
