...
export const infoserverEnvSchema = z.object({
  ...
  RATE_LIMIT_EXPORT_PER_MINUTE: z.coerce.number().int().min(1).default(60).describe("Full-catalog requests allowed per client per minute, spendable all at once. Every client fetches the whole catalog once per refresh, and processes behind one address share a bucket, so this has to clear several of them at once rather than just one").meta(expert()),
