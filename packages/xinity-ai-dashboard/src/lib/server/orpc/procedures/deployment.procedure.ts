    ...
    Promise.all(
      modelsToCheck.map(async (m) => {
        ...
        return {
          ...
          blockedVersions: info.blockedVersions,
  ...
  for (const model of resolved) {
    ...
    const compatible = remaining.filter(n => {
      const req: ModelNodeRequirements = {
        ...
        minVersion: model.minVersion, blockedVersions: model.blockedVersions,
...
export const createDeployment = rootOs
  ...
  .handler(async ({ context, input, errors }) => {
    ...
    await assertModelsInCatalog(
      [primarySpecifier, ...(earlySpecifier ? [earlySpecifier] : [])],
      errors,
    );
    ...
    try {
      ...
      void notifyOrgMembers({
        ...
        data: {
          ...
          orgName: orgName ?? "",
          dashboardUrl: `${config.server.origin}/modelhub/`,
        },
