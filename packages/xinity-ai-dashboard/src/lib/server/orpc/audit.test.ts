...
const verifyApiKey = mock(async () => ({ valid: true, key: { id: "key-1" } }));
...
describe("runWithAudit", () => {
  beforeEach(() => {
    ...
    verifyApiKey.mockImplementation(async () => ({ valid: true, key: { id: "key-1" } }));
  });
  ...
  test("writes an instance-scoped event (null org) when no organization is in context", async () => {
    const result = await runWithAudit(
      ctx({ session: { user: { id: "user-1" } }, activeOrganizationId: undefined }),
      auditTag,
      async () => "OK",
    );
    expect(result).toBe("OK");
  ...
  test("attributes the event to an API key when x-api-key is present", async () => {
    const c = ctx({
      request: new Request("http://localhost/rpc", { headers: { "x-api-key": "secret", "user-agent": "test-agent" } }),
    });
