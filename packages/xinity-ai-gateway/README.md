...
| `WEB_SEARCH_PROVIDER` | (unset) | Web search backend: `searxng`, `google`, `bing`, `brave`, or `serper`. Web search is disabled when unset. |
| `WEB_SEARCH_CREDENTIAL` | (unset) | Provider credential: instance URL for SearXNG, `apikey:cx` for Google, API key for Bing/Brave/Serper |
